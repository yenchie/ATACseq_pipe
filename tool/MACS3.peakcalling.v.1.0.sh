...
## run cutoff-analysis ------
if [ -f ./MACS3/$sampleID.poisson.ATAC_cutoff_analysis.tsv ]; then
    echo "Cutoff analysis already done, skipping..."
else
    time macs3 hmmratac \
    ...
    --hmm-type poisson 2> macs3.prerun.log
fi
# The cutoff analysis will generate a file named $sampleID.poisson.ATAC_cutoff_analysis.tsv
cut_offs=$(awk 'NR > 1 && $4 >= 500 && $4 <= 1000 {print $1}' "./MACS3/$sampleID.poisson.ATAC_cutoff_analysis.tsv" | awk 'NR==1 {first=$1} {last=$1} END {print first, last}')
echo "Cut-offs: $cut_offs"
upper=$(echo "$cut_offs" | cut -f1 -d$' ')
lower=$(echo "$cut_offs" | cut -f2 -d$' ')
upper=${upper/.00/}
lower=${lower/.00/}
...
for bdg in $(find ./MACS3/ -name "*digested*.bdg"); do
    # remove path and .bdg extension
    bdg_prefix=$(basename "$bdg" .bdg)
...
profile_ref="/bcst/JYL/JYL_qnap_2/YCWang/testing/macs3_test/sampled.gene.ref.bed"
echo "profile_ref: $profile_ref"
time bash /bcst/JYL/JYL_qnap_2/YCWang/0_Script/000/Pipeline/ATAC_pipe/tool/deeptools.heatmap.4.macs3.splited.bws.v0.1.sh $sampleID ./ $profile_ref
